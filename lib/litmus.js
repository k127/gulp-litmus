...
Litmus.prototype.logHeaders = function(data) {
  ...
  console.log('---------------------\n'
   //+ body  // suppress log output of body
   );
...
/**
* Save response body to file once email is sent
*
* @param {Array} data - array of data returned from promise
*
*/
Litmus.prototype.saveBody = function(data) {
  ...
  if (status > 199 && status < 300) {
    ...
    var map      = {},
        ...
        id       = $('test_set').children('id').text(),
        filePath = 'log/' + slugify(this.title)
                     + '.' + id + '.xml';

    ensureDirectoryExistence(filePath);
    fs.writeFileSync(filePath, body);

    // TODO further parse body
    // TODO load litmus client screenshots
